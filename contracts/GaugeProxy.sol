...
contract Gauge is ReentrancyGuard {
    ...
    IERC20 public constant DOP = IERC20(0x7CeBC7A14ff1399C53Fe1d0b19CC4388b5eA7433);
    IERC20 public constant VeDop = IERC20(0x431F7FEC6E77E8efD7bA10813DF061DAcf84015f);
    ...
    function derivedBalance(address account) public view returns (uint) {
        ...
        uint _adjusted = (_totalSupply.mul(VeDop.balanceOf(account)).div(VeDop.totalSupply())).mul(60).div(100);
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            DOP.safeTransfer(msg.sender, reward);
    ...
    function notifyRewardAmount(uint256 reward) external onlyDistribution updateReward(address(0)) {
        DOP.safeTransferFrom(DISTRIBUTION, address(this), reward);
        ...
        uint balance = DOP.balanceOf(address(this));
...
contract MasterVeDop {
    using SafeMath for uint;

    /// @notice EIP-20 token name for this token
    string public constant name = "Master VeDOP";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "mveDop";
...
contract GaugeProxy is ProtocolGovernance {
    ...
    constructor(address _master, address _veDop, address _dop) public {
        TOKEN = IERC20(address(new MasterVeDop()));
    ...
    // Adjusts _owner's votes according to latest _owner's VeDop balance
    function poke(address _owner) public {
        ...
        uint256 _weight = VeDop.balanceOf(_owner);        
    ...
    // Deposits mVeDop into MasterChef
    ...
    function distribute() external {
        collect();
        uint _balance = DOP.balanceOf(address(this));
        if (_balance > 0 && totalWeight > 0) {
            for (uint i = 0; i < _tokens.length; i++) {
                ...
                if (_reward > 0) {
                    DOP.safeApprove(_gauge, 0);
                    DOP.safeApprove(_gauge, _reward);
